<script>
	...
	import MyPortfolioIcon from './MyPortfolioIcon.svelte';
...
<header>
	<nav
		...
		aria-label="Main"
	>
		<a href="/"><MyPortfolioIcon /><span class="sr-only">LinkedIN Profile</span></a>

		<ul class="flex gap-6">
			{#each settings.data.navigation as item (item.label)}<li>
					{#if item.cta_button}
